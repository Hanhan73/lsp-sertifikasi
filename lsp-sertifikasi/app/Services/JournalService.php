...
class JournalService
{
    ...
    {
        $akunPiutang = $piutang->coa; // ChartOfAccount object

        if ($piutang->jenis === 'pinjaman') {
            $kodeLawan = '1-002';
        } else {
            // jenis tagihan: pakai coa_lawan_id yang dipilih user
            $akunLawan = $piutang->coaLawan ?? ChartOfAccount::where('kode', '4-002')->firstOrFail();
            $kodeLawan = $akunLawan->kode;
        }
        ...
        return $this->createJournal(
            ...
            refId: $piutang->id,
            lines: [
                ['akun' => $akunPiutang->kode, 'debit' => $piutang->jumlah, 'kredit' => 0, 'ket' => $piutang->uraian],
                ['akun' => $kodeLawan,         'debit' => 0, 'kredit' => $piutang->jumlah,  'ket' => $piutang->nama_pihak],
            ]
        );
    }
    ...
    public function jurnalPiutangLainnyaLunas(OtherReceivable $piutang): JournalEntry
    {
        ...
        return $this->createJournal(
            ...
            refId: $piutang->id,
            lines: [
                ['akun' => '1-002',            'debit' => $piutang->jumlah_lunas, 'kredit' => 0,                      'ket' => "Kas masuk dari {$piutang->nama_pihak}"],
                ['akun' => $akunPiutang->kode, 'debit' => 0,                     'kredit' => $piutang->jumlah_lunas, 'ket' => $piutang->uraian],
            ]
        );
    }
        ...
        return $this->createJournal(
            ...
            lines: [
                ...
                [
                    'akun' => '4-001',
                    'debit' => 0,
                    'kredit' => (int)$invoice->total_amount,
