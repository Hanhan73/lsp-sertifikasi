...
@push('styles')
<style>
.schedule-card { border: 1px solid #e2e8f0; border-radius: 12px; transition: box-shadow .2s; background: #fff; overflow: hidden; margin-bottom: 12px; }
.schedule-card:hover { box-shadow: 0 4px 18px rgba(0,0,0,.09); }
.schedule-header { padding: 14px 18px 12px; border-bottom: 1px solid #f1f5f9; background: linear-gradient(135deg, #f8fafc 0%, #fff 100%); cursor: pointer; user-select: none; }
.schedule-body { padding: 0; }
.status-pill { display: inline-flex; align-items: center; gap: 4px; padding: 3px 10px; border-radius: 20px; font-size: .73rem; font-weight: 600; }
...
.doc-badge { display: inline-flex; align-items: center; gap: 4px; padding: 3px 9px; border-radius: 20px; font-size: .7rem; font-weight: 600; text-decoration: none; }
.doc-badge:hover { opacity: .85; }
.needs-action { animation: pulse-warn 2s ease-in-out infinite; }
@keyframes pulse-warn { 0%,100% { box-shadow: 0 0 0 0 rgba(245,158,11,.4); } 50% { box-shadow: 0 0 0 6px rgba(245,158,11,0); } }
.filter-bar { background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 12px 16px; margin-bottom: 18px; display: flex; gap: 10px; align-items: center; flex-wrap: wrap; }
...
<div class="filter-bar">
    ...
    <input type="text" id="filter-search" class="form-control form-control-sm" placeholder="Cari nama asesi…" style="width:180px;">
</div>
...
@php
    ...
    $pendingFrak01InSched = $schedule->asesmens->filter(fn($a) => $a->frak01?->status === 'draft')->count();
    $hasUrgent = ($pendingApl01InSched + $pendingApl02InSched) > 0;
@endphp

<div class="schedule-card schedule-item" data-tuk="{{ $schedule->tuk_id }}" data-status="{{ $statusKey }}">
    ...
    <div class="schedule-header d-flex align-items-center gap-3" onclick="toggleSchedule(this)">
        ...
        <div class="flex-grow-1">
            <div class="d-flex align-items-center gap-2 flex-wrap">
                ...
                @if($pendingFrak01InSched > 0)
                <span class="badge bg-secondary needs-action" style="font-size:.68rem;">
                    <i class="bi bi-file-earmark-check me-1"></i>{{ $pendingFrak01InSched }} FR.AK.01 Draft
                </span>
                @endif
            ...
            <table class="table table-sm align-middle mb-0">
                <thead>
                    <tr style="background:#f8fafc; font-size:.78rem; color:#64748b; text-transform:uppercase; letter-spacing:.4px;">
                        ...
                        <th style="padding:10px 14px;" colspan="3">Dokumen</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($schedule->asesmens as $idx => $asesi)
                    @php
                        $ak01 = $asesi->frak01 ?? null;
                        $apl01 = $asesi->aplsatu;
                        $apl02 = $asesi->apldua;
                    @endphp
                    <tr class="asesi-row border-bottom"
                        ...
                        <td style="padding:10px 8px;" onclick="event.stopPropagation()">
                            @if($ak01)
                            @php
                                $ak01Color = match($ak01->status) {
                                    'verified','approved' => 'bg-success text-white',
                                    'submitted' => 'bg-info text-white',
                                    default => 'bg-secondary text-white',
                                };
                            @endphp
                            @if(in_array($ak01->status, ['verified','approved']))
                            <a href="{{ route('admin.frak01.pdf', $ak01) }}?preview=1"
                               target="_blank"
                               class="doc-badge {{ $ak01Color }}"
                               title="Lihat PDF FR.AK.01">
                                <i class="bi bi-file-earmark-check"></i>
                                AK.01: {{ ucfirst($ak01->status) }}
                            </a>
                            @else
                            <span class="doc-badge {{ $ak01Color }}">
                                <i class="bi bi-file-earmark-check"></i>
                                AK.01: {{ ucfirst($ak01->status) }}
                            </span>
                            @endif
                            ...
                            <span class="doc-badge bg-light text-muted border">
                                <i class="bi bi-file-earmark-check"></i>AK.01: -
                            </span>
                            @endif
                        </td>

                        {{-- FR.AK.04 badge (banding) --}}
                        <td style="padding:10px 8px;" onclick="event.stopPropagation()">
                            @php $ak04 = $asesi->frak04 ?? null; @endphp
                            @if($ak04 && $ak04->status === 'submitted')
                            <a href="{{ route('admin.frak04.pdf', $ak04) }}?preview=1"
                            target="_blank"
                            class="doc-badge bg-warning text-dark"
                            title="Lihat PDF FR.AK.04 Banding">
                                <i class="bi bi-megaphone"></i>
                                AK.04: Banding
                            </a>
                            @else
                            <span class="doc-badge bg-light text-muted border" title="Tidak ada banding">
                                <i class="bi bi-megaphone"></i>AK.04: -
                            </span>
                            @endif
                        </td>
...
function applyFilters() {
    ...
    document.querySelectorAll('.schedule-item').forEach(card => {
        ...
        let   okSearch = !search;
        if (search) { card.querySelectorAll('tbody tr').forEach(r => { if (r.textContent.toLowerCase().includes(search)) okSearch = true; }); }
