...
.stat-card { border:1.5px solid #e2e8f0; border-radius:10px; padding:16px 18px; background:#fff; transition:box-shadow .2s, border-color .2s; }
...
.ready-group { border:1px solid #e2e8f0; border-radius:8px; overflow:hidden; margin-bottom:10px; }
.ready-group-header { background:#f8fafc; padding:8px 14px; border-bottom:1px solid #e2e8f0; display:flex; align-items:center; gap:10px; }
.ready-asesi-row { display:flex; align-items:center; gap:10px; padding:8px 14px; border-bottom:1px solid #f8fafc; font-size:.83rem; }
...
.avatar-stack .av { width:26px; height:26px; border-radius:50%; background:#e0e7ff; color:#4f46e5; font-size:.65rem; font-weight:700; display:flex; align-items:center; justify-content:center; border:2px solid #fff; margin-left:-6px; flex-shrink:0; }
.avatar-stack .av:first-child { margin-left:0; }
.avatar-stack .av-more { background:#f1f5f9; color:#64748b; }

.loc-pill-online  { background:#eff6ff; color:#2563eb; border:1px solid #bfdbfe; border-radius:99px; padding:1px 8px; font-size:.65rem; font-weight:600; white-space:nowrap; display:inline-flex; align-items:center; gap:3px; }
.loc-pill-offline { background:#f1f5f9; color:#475569; border:1px solid #e2e8f0; border-radius:99px; padding:1px 8px; font-size:.65rem; font-weight:600; white-space:nowrap; display:inline-flex; align-items:center; gap:3px; }
</style>
@endpush
...
<div class="row g-3 mb-4">
    <div class="col-6 col-md-3">
        <div class="stat-card"><div class="stat-num text-primary">{{ $totalSchedules }}</div><div class="stat-lbl">Total Jadwal</div></div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card"><div class="stat-num text-warning">{{ $totalReady }}</div><div class="stat-lbl">Menunggu Dijadwalkan</div></div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card"><div class="stat-num text-success">{{ $upcomingCount }}</div><div class="stat-lbl">Akan Datang</div></div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card"><div class="stat-num text-danger">{{ $noAsesorCount }}</div><div class="stat-lbl">Belum Ada Asesor</div></div>
    </div>
</div>
...
<div class="alert alert-warning d-flex align-items-center gap-2 mb-4 py-2">
    ...
    <span><strong>{{ $pendingApprovalCount }} jadwal</strong> sedang menunggu persetujuan Direktur.</span>
</div>
@endif
...
<div class="col-lg-4">
    <div class="card border-0 shadow-sm h-100">
        <div class="card-header bg-white border-bottom d-flex align-items-center gap-2">
            <div class="rounded-circle d-flex align-items-center justify-content-center" style="width:26px;height:26px;background:#fef9c3;">
                ...
                <div class="ready-asesi-row">
                    ...
                    <div class="flex-grow-1 min-width-0">
                        ...
                        <div class="text-muted" style="font-size:.72rem;">
                            {{ $asesmen->skema?->name ?? '-' }}
                            @if($asesmen->is_collective) &bull; <span class="badge bg-primary" style="font-size:.6rem;">Kolektif</span>@endif
                        </div>
                    </div>
                </div>
                @endforeach
                    ...
                    <tbody>
                        ...
                        <tr class="sched-row"
                                ...
                                <div class="d-flex align-items-start gap-2">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-muted" style="font-size:.75rem;">
                                            ...
                                            <a href="{{ $schedule->meeting_link }}" target="_blank" class="ms-1" onclick="event.stopPropagation()"><i class="bi bi-box-arrow-up-right" style="font-size:.65rem;"></i></a>
                                            @endif
                                        </div>
                                        <div class="mt-1">
                                            <span class="{{ $isOnline ? 'loc-pill-online' : 'loc-pill-offline' }}">
                            ...
                            <td>
                                ...
                                <div class="text-muted" style="font-size:.75rem;"><i class="bi bi-building me-1"></i>{{ $schedule->tuk?->name ?? '-' }}</div>
                            </td>
                            ...
                            <td class="text-center">
                                <div class="avatar-stack justify-content-center">
                                    ...
                                    <div class="av" title="{{ $a->full_name }}">{{ strtoupper(substr($a->full_name, 0, 1)) }}</div>
                                    @endforeach
                                    @if($peserta > 3)<div class="av av-more">+{{ $peserta - 3 }}</div>@endif
                                </div>
                            ...
                            {{-- Status --}}
                            <td class="text-center">
                                @if($isToday)<span class="badge bg-info">Berlangsung</span>
                                @elseif($isPast)<span class="badge bg-secondary">Selesai</span>
                                @else<span class="badge bg-success">Akan Datang</span>@endif
                            </td>

                            {{-- Approval --}}
                            <td class="text-center">
                                <span class="badge bg-{{ $schedule->approval_status_badge }}">{{ $schedule->approval_status_label }}</span>
                            </td>

                            {{-- Aksi --}}
                            <td class="text-end pe-3" onclick="event.stopPropagation()">
                                <div class="d-flex justify-content-end gap-1">
                                    <a href="{{ route('admin.schedules.show', $schedule) }}" class="btn btn-sm btn-outline-primary" title="Lihat Detail"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('admin.schedules.edit', $schedule) }}" class="btn btn-sm btn-outline-secondary" title="Edit"><i class="bi bi-pencil"></i></a>
                                    <button type="button" class="btn btn-sm btn-outline-danger" title="Hapus" onclick="deleteSchedule({{ $schedule->id }}, {{ $peserta }})"><i class="bi bi-trash3"></i></button>
                                </div>
                            </td>
                        </tr>
                        @endforeach
...
async function deleteSchedule(id, peserta) {
    const result = await Swal.fire({
        title: 'Hapus Jadwal?',
        html: `<p class="text-muted small mb-2">Jadwal ini akan dihapus dan <strong>${peserta} asesi</strong> akan dikembalikan ke status sebelumnya.</p>
               <div class="alert alert-warning py-2 small mb-0"><i class="bi bi-exclamation-triangle me-1"></i>Tindakan ini tidak dapat dibatalkan.</div>`,
