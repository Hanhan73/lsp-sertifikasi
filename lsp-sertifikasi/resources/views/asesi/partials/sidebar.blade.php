...
@if($asesmen->payment || ($asesmen->is_collective && in_array($asesmen->status, ['verified', 'paid', 'scheduled',
...
@if($asesmen->schedule)
<a href="{{ route('asesi.schedule') }}" class="nav-link {{ Str::startsWith($currentRoute, 'asesi.schedule') || Str::startsWith($currentRoute, 'asesi.apl01') ? 'active' : '' }}">
    ...
    @endif
    @php $aplStatus = $asesmen->aplsatu?->status; @endphp
    @if($aplStatus === 'draft')
...
<a href="{{ route('asesi.result') }}" class="nav-link {{ $currentRoute == 'asesi.result' ? 'active' : '' }}">
...
<a href="{{ route('asesi.documents') }}" class="nav-link {{ $currentRoute == 'asesi.documents' ? 'active' : '' }}">
...
<a href="{{ route('asesi.batch-info') }}" class="nav-link {{ $currentRoute == 'asesi.batch-info' ? 'active' : '' }}">
