    ...
    <div class="card-body py-3">
        <div class="d-flex align-items-center gap-3 flex-wrap">
            ...
            <div class="ms-auto d-flex gap-2">
                <a href="{{ route('direktur.keuangan.download.rekap-pendapatan', ['tahun' => $tahun, 'format' => 'pdf']) }}"
                   class="btn btn-sm btn-outline-danger">
                    <i class="bi bi-file-earmark-pdf"></i> Export PDF
                </a>
                <a href="{{ route('direktur.keuangan.download.rekap-pendapatan', ['tahun' => $tahun, 'format' => 'excel']) }}"
...
<div class="row g-3 mb-4">
    <div class="col-lg-6">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white fw-semibold border-bottom">
                <i class="bi bi-pie-chart text-success me-2"></i>Komposisi Pengeluaran
            </div>
    ...
    <div class="col-lg-6">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white fw-semibold border-bottom">
                <i class="bi bi-bar-chart text-warning me-2"></i>Trend Pemasukan vs Pengeluaran
            </div>
            <div class="card-body">
            ...
            <table class="table table-hover table-bordered mb-0 align-middle" style="font-size:.875rem;">
                ...
                <tr @if($ada) onclick="toggleBulan({{ $m }})" style="cursor:pointer" @endif
                    ...
                    <td class="text-center">
                        @if($ada)
                        <i class="bi bi-chevron-right text-secondary" id="chevron-{{ $m }}"
                           style="transition:transform .2s;font-size:.8rem;"></i>
                        @endif
                    </td>
                    <td class="fw-semibold">{{ $bln }}</td>
                    <td class="text-end text-success">{{ $in > 0 ? 'Rp '.number_format($in,0,',','.') : '-' }}</td>
                    <td class="text-end text-danger">{{ $hon > 0 ? 'Rp '.number_format($hon,0,',','.') : '-' }}</td>
                    <td class="text-end text-info">{{ $ops > 0 ? 'Rp '.number_format($ops,0,',','.') : '-' }}</td>
                    <td class="text-end">{{ ($hon+$ops) > 0 ? 'Rp '.number_format($hon+$ops,0,',','.') : '-' }}</td>
                        ...
                        <div class="px-4 py-3">
                            <table class="table table-sm table-bordered mb-0 bg-white" style="font-size:.82rem;">
                                ...
                                <tr>
                                    ...
                                    <td class="text-end fw-semibold {{ $r['tipe'] === 'pemasukan' ? 'text-success' : 'text-danger' }}">
                                        {{ $r['tipe'] === 'pemasukan' ? '+' : '−' }}
                                        Rp {{ number_format($r['jumlah'],0,',','.') }}
                                    </td>
                                </tr>
                                @empty
                                <tr><td colspan="5" class="text-center text-muted py-2">Tidak ada transaksi</td></tr>
                                @endforelse
                                </tbody>
            ...
            <div class="card-body p-0">
                <table class="table table-sm table-hover mb-0" style="font-size:.82rem;">
                    <thead class="table-light">
                        <tr><th>TUK</th><th class="text-center">Trx</th><th class="text-end">Total</th></tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end fw-semibold text-success">Rp {{ number_format($row->total,0,',','.') }}</td>
                        </tr>
                        @empty
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm h-100">
            ...
            <div class="card-body p-0">
                <table class="table table-sm table-hover mb-0" style="font-size:.82rem;">
                    <thead class="table-light">
                        <tr><th>Skema</th><th class="text-center">Trx</th><th class="text-end">Total</th></tr>
                    </thead>
                    <tbody>
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-header bg-white d-flex justify-content-between align-items-center border-bottom">
        <span class="fw-semibold">
            <i class="bi bi-clock-history text-secondary me-2"></i>50 Transaksi Terverifikasi Terbaru
        </span>
    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover table-sm mb-0 align-middle" style="font-size:.82rem;">
                <thead class="table-light">
                    <tr>
                        <th>Tanggal</th><th>Nama Asesi</th><th>TUK</th>
                        <th>Skema</th><th class="text-center">Jenis</th><th class="text-end">Jumlah</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge bg-primary" style="font-size:.7rem;">{{ Str::limit($p->asesmen->skema->name ?? '-', 25) }}</span></td>
                        <td class="text-center">
...
new Chart(document.getElementById('chartTrend'), {
    ...
    data: {
        labels: bulanLabels,
        datasets: [
            { label: 'Pemasukan', data: dataPemasukan, backgroundColor: 'rgba(17,153,142,.75)', borderRadius: 5 },
            { label: 'Pengeluaran', data: dataPemasukan.map((v, i) => dataHonor[i] + dataBiayaOps[i]), backgroundColor: 'rgba(245,87,108,.65)', borderRadius: 5 },
        ],
    },
    options: {
