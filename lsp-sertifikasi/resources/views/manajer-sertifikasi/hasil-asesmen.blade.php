...
@section('breadcrumb', 'Distribusi › Hasil Asesmen')
...
<div class="mb-4">
    <div class="d-flex align-items-center gap-2 mb-2">
        ...
        <a href="{{ route('manajer-sertifikasi.distribusi') }}"
           class="btn btn-sm btn-outline-secondary">
            <i class="bi bi-grid me-1"></i>Semua Jadwal
        </a>
    </div>
    ...
    <div class="d-flex align-items-start justify-content-between flex-wrap gap-3">
        ...
        <div class="d-flex flex-wrap gap-2 align-items-start">
            ...
            <div class="text-center px-3 py-2 rounded-3" style="background:#eff6ff;border:1px solid #bfdbfe;min-width:70px">
                <div class="fw-bold" style="font-size:1.2rem;color:#2563eb">{{ $hasilObservasi->count() }}/{{ $totalObservasi }}</div>
            ...
            <div class="text-center px-3 py-2 rounded-3" style="background:#fdf4ff;border:1px solid #e9d5ff;min-width:70px">
                <div class="fw-bold" style="font-size:1.2rem;color:#7c3aed">{{ $hasilPortofolio->count() }}/{{ $totalPortofolio }}</div>
...
{{-- ===== REKAP NILAI PER ASESI (TABEL BESAR) ===== --}}
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0" style="font-size:.85rem;">
                ...
                <tbody>
                    ...
                    @php
                        // Teori
                        $soalAsesi = $distribusiTeori
                            ? $distribusiTeori->soalAsesi->where('asesmen_id', $asesmen->id)
                            : collect();
                        $totalSoal = $soalAsesi->count();
                        $benarSoal = $soalAsesi->filter(fn($s) =>
                    ...
                    <tr>
                        ...
                        {{-- Observasi (link GDrive per asesi) --}}
                        @foreach($schedule->distribusiSoalObservasi as $distObs)
                        @php
                        ...
                        {{-- Portofolio per asesi (placeholder — hasil dinilai asesor) --}}
                        @foreach($schedule->distribusiPortofolio as $distPorto)
                        @php
                            $hasil = $hasilPortofolio->where('portofolio_id', $distPorto->portofolio_id)->first();
                        @endphp
                        <td class="text-center">
                            @if($hasil)
...
<div class="row g-4">
    ...
    {{-- ===== KIRI: Daftar Hadir + Nilai Teori Detail ===== --}}
    <div class="col-lg-6">
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white d-flex align-items-center justify-content-between py-3">
                ...
                @else
                <span class="badge bg-secondary" title="Asesor belum memiliki TTD">Belum TTD</span>
                @endif
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white py-3">
                <div class="fw-semibold">
                    <i class="bi bi-journal-text text-primary me-2"></i>Detail Nilai Soal Teori
                    @if($distribusiTeori)
                        <span class="text-muted fw-normal" style="font-size:.8rem;">
                            ({{ $distribusiTeori->jumlah_soal }} soal · {{ $distribusiTeori->durasi_menit ?? 30 }} menit)
                        </span>
                    @endif
                </div>
            </div>
            <div class="card-body p-0">
                ...
                <div class="text-center py-4 text-muted small">
                    <i class="bi bi-exclamation-circle me-1"></i>Soal teori belum didistribusikan
                </div>
                @else
                <div class="table-responsive">
                    <table class="table table-sm align-middle mb-0" style="font-size:.83rem;">
                        ...
                        <tbody>
                            ...
                            @php
                                $soalA = $distribusiTeori->soalAsesi->where('asesmen_id', $asesmen->id);
                                $tot   = $soalA->count();
                                $ben   = $soalA->filter(fn($s) => $s->jawaban && $s->jawaban === $s->soalTeori?->jawaban_benar)->count();
                                $sub   = $soalA->whereNotNull('submitted_at')->isNotEmpty();
                                $pct   = $tot > 0 ? round($ben / $tot * 100) : 0;
                            @endphp
                            <tr>
                                <td class="ps-3 fw-semibold">{{ $asesmen->full_name }}</td>
                                <td class="text-center {{ $sub ? ($pct >= 60 ? 'text-success fw-bold' : 'text-danger fw-bold') : 'text-muted' }}">
                                    {{ $sub ? $ben : '—' }}
                                </td>
                                ...
                                <td class="text-center">
                                    ...
                                    <div class="d-flex align-items-center gap-1 justify-content-center">
                                        <div class="progress flex-grow-0" style="height:5px;width:50px;">
                                ...
                                <td class="text-center">
                                    @if(!$tot)
                                        <span class="badge bg-secondary" style="font-size:.65rem;">Belum ada soal</span>
                                    @elseif(!$sub)
                                        <span class="badge bg-warning text-dark" style="font-size:.65rem;">Belum Submit</span>
                                    @elseif($pct >= 60)
                                        <span class="badge bg-success" style="font-size:.65rem;">Lulus</span>
                                    @else
                                        <span class="badge bg-danger" style="font-size:.65rem;">Tidak Lulus</span>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
    ...
    {{-- ===== KANAN: Observasi + Portofolio + Berita Acara ===== --}}
    <div class="col-lg-6">
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white py-3">
                <div class="fw-semibold">
                    ...
                    <span class="badge bg-secondary ms-1">{{ $hasilObservasi->count() }}/{{ $totalObservasi }} diupload asesor</span>
                </div>
            </div>
            <div class="list-group list-group-flush">
                ...
                <div class="list-group-item px-4 py-3">
                    <div class="d-flex align-items-start gap-3">
                        <i class="bi {{ $hasil ? 'bi-check-circle-fill text-success' : 'bi-circle text-muted' }} fs-5 flex-shrink-0 mt-1"></i>
                        <div class="flex-grow-1">
                            <div class="fw-semibold small">{{ $obs->judul }}</div>
                            @if($paket)
                            <div class="text-muted" style="font-size:.75rem;">
                                <i class="bi bi-collection me-1"></i>Paket {{ $paket->kode_paket }}
                            </div>
                            @endif
                            @if($hasil)
                            <div class="text-muted mt-1" style="font-size:.75rem;">
                                <i class="bi bi-file-earmark me-1"></i>{{ $hasil->file_name }}
                                · <i class="bi bi-clock me-1"></i>{{ $hasil->uploaded_at->translatedFormat('d M Y, H:i') }}
                                @if($hasil->catatan)
                                    <br><i class="bi bi-chat-text me-1"></i>{{ $hasil->catatan }}
                                @endif
                            </div>
                            @else
                            ...
                            <div class="mt-2">
                                <div class="text-muted" style="font-size:.72rem;font-weight:600;text-transform:uppercase;letter-spacing:.03em;">
                                    Link Google Drive Asesi:
                                </div>
                                @foreach($linkAsesi as $jwb)
                                @php $namaAsesi = $schedule->asesmens->find($jwb->asesmen_id); @endphp
                                <div class="d-flex align-items-center gap-2 mt-1">
                                    <span class="text-muted small flex-shrink-0" style="min-width:120px;">
                                        {{ $namaAsesi?->full_name ?? '—' }}
                                    </span>
                                    <a href="{{ $jwb->gdrive_link }}" target="_blank"
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white d-flex align-items-center justify-content-between py-3">
                ...
                <div class="d-flex gap-2">
                    @if($beritaAcara)
                    <a href="{{ route('manajer-sertifikasi.jadwal.berita-acara.pdf', $schedule) }}"
                    target="_blank" class="btn btn-sm btn-outline-danger">
            ...
            <div class="card-body text-center py-5 text-muted">
                ...
                <small>Asesor akan mengisi berita acara setelah proses asesmen selesai</small>
            </div>
            @else
            <div class="card-body">
                {{-- Info BA --}}
                <div class="d-flex flex-wrap gap-3 mb-3 p-3 rounded-3 bg-light border">
                    <div>
                        <div class="text-muted" style="font-size:.72rem;font-weight:600;text-transform:uppercase;">Tanggal Pelaksanaan</div>
                        <div class="fw-semibold small">{{ $beritaAcara->tanggal_pelaksanaan?->translatedFormat('d F Y') ?? '-' }}</div>
                    </div>
                    @if($beritaAcara->catatan)
                    <div>
                        <div class="text-muted" style="font-size:.72rem;font-weight:600;text-transform:uppercase;">Catatan</div>
                        <div class="small">{{ $beritaAcara->catatan }}</div>
                    </div>
                    @endif
                    @if($beritaAcara->signed_at ?? null)
                    <div>
                        <div class="text-muted" style="font-size:.72rem;font-weight:600;text-transform:uppercase;">Ditandatangani</div>
                        <div class="fw-semibold small text-success">
                            <i class="bi bi-check-circle-fill me-1"></i>
                            {{ \Carbon\Carbon::parse($beritaAcara->signed_at)->translatedFormat('d M Y, H:i') }}
                        </div>
                    </div>
                    @endif
                </div>
                    ...
                    <div class="d-flex align-items-center gap-3 p-2 rounded-3 border
                        ...
                        @if($rek === 'K')
                            <span class="badge bg-success px-3 py-2">
                                <i class="bi bi-award me-1"></i>Kompeten
                            </span>
                        @elseif($rek === 'BK')
                ...
                <div class="d-flex gap-3 mt-3 p-3 rounded-3" style="background:#f8fafc;border:1px solid #e2e8f0;">
                    ...
                    <div class="text-center flex-fill">
                        ...
                        <div class="text-muted" style="font-size:.72rem;font-weight:600;">TOTAL ASESI</div>
                    </div>
                </div>
                @endif
            </div>
