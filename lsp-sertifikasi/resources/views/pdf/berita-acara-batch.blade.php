...
<body>
    ...
    /**
    * Bangun baris dengan border logic:
    *
    * Kolom asesor:
    * - is_first → tampilkan nama asesor, border-top hitam, border-bottom PUTIH
    * - is_middle → kosong, border-top & bottom PUTIH
    * - is_last → kosong, border-top PUTIH, border-bottom HITAM ← garis penutup grup
    * - is_only → tampilkan nama asesor, border semua sisi HITAM (grup 1 baris)
    *
    * border-left & border-right selalu HITAM (kolom tetap ada)
    */
    $baris = collect();
    $noUrut = 1;
    ...
    // border-top kolom asesor
    $btop = ($isFirst || $isOnly) ? '#000' : '#fff';
    // border-bottom kolom asesor: hitam hanya di baris terakhir grup
    $bbottom = ($isLast || $isOnly) ? '#000' : '#fff';

    $baris->push([
    'no' => $noUrut++,
    'nama' => $asesmen->full_name,
    'asesor' => ($isFirst || $isOnly) ? $asesor : null,
    'btop' => $btop,
    'bbottom' => $bbottom,
    ...
    <table class="tabel-peserta">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $b['nama'] }}</td>
                <td style="
                    border-top:    1pt solid {{ $b['btop'] }};
                    border-bottom: 1pt solid {{ $b['bbottom'] }};
